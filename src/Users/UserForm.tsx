...
type UserFormProps = {
    onSubmitHandler: (name: string, age: number) => void;
    onCancel: () => void;
}
...
const UserForm = ({ onSubmitHandler, onCancel }: UserFormProps) => {
    ...
    const submitHandler = (e: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        if (!userRef.current?.value || !ageRef.current?.value) return;

        onSubmitHandler(userRef.current!.value, +ageRef.current!.value);
    }
    return (
        <div>
